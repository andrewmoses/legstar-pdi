 ...
 * Here Kettle should have registered our plugin on startup.
 * Can't inherit from TransformationTestCase here because its
 * constructor initializes Kettle and then it is too late to
...
public class ZosFileITCase extends TestCase {
    ...
    /**
     * {@inheritDoc}
     * We tell Kettle to look at our target folder for plugins
     * to load into the registry.
     * 
     * */
    ...
    /**
     * Create a transformation, generate the transformers and run them.
     * This simulates the complete process as if performed interactively.
     * @throws Exception if test fails
     */
    public void testGenerateAndExecute() throws Exception {
        ...
        String compositeJaxbClassName = CobolToPdi.getCompositeJaxbClassName(ClassUtil
        ...
        ZosFileInputMeta zosFileInputMeta = (ZosFileInputMeta) registry.loadClass(sp);
        zosFileInputMeta.setCompositeJaxbClassName(compositeJaxbClassName);
        
        // Get the meta fields descriptions
        zosFileInputMeta.setInputFields(
                CobolToPdi.getCobolFields(compositeJaxbClassName, getClass()));
        ...
        // Setup the a PDI Transformation [injector -> zosFileInput -> dummy] and run it
        ...
        List<RowMetaAndData> result = TransTestFactory
                .executeTestTransformation(transMeta,
                        TransTestFactory.INJECTOR_STEPNAME, zosFileInputStepName,
    ...
    /**
     * Creates a classpath with all legstar-pdi dependencies.
     * This is needed because transformers generation process needs to
     * compile classes and typically javac does not inherit class loaders.
    ...
    /*
     * ------------------------------------------------------------------------
     * Following code is duplicate from org.pentaho.di.trans.TransformationTestCase.
     * ------------------------------------------------------------------------
     */
